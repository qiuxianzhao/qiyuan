<?php 
    ...
    if($_POST){
       ...
       $email = htmlspecialchars($_POST['email']);
        $sql = "insert into `student` (`name`, `avator`, `username`, `password`, `status`, `sex`, `age`, `addtime`, `class_id`, `desc`, `fav`, `mobi`, `email`) values ('$name','$avator','$username','$psw',$status,$sex,$age,$addtime,$class,'$desc','$fav','$phone','$email')";
        $res = mysqli_query($con,$sql);
        if($res && mysqli_affected_rows($con)){
            showMsg('添加学生成功！','show-student.php');
        }else{
            showMsg('添加学生失败');
        }
    }
...
<body>
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <li><label>入学时间</label><input name="addtime" id="addtime" type="text" class="dfinput" value="<?php echo date('Y-m-d H:i:s'); ?>" /></li>
