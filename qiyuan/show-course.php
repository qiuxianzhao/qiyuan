<?php 
    ...
    if(isset($_GET['del'])){
        $del = $_GET['del'];
        $sql = "select cover from course where id in ($del)";
        $del_img = getOne($con,$sql)['cover'];
        $sql = "delete from course where id in ($del)";
        $res = mysqli_query($con,$sql);
        if( $res && mysqli_affected_rows($con) ){
            if(file_exists(ROOT.$del_img)){
                unlink(ROOT.$del_img);
            }
            showMsg('删除成功！','show-course.php');
        }else{
            showMsg('删除失败！');
        }
    }
    ...
    $sql = "select count('id') c from course";
    ...
    $sql = "select * from course limit ".($pagenow-1)*$pagesize.",$pagesize";
    $data = getAll($con,$sql);
    mysqli_close($con);
 ?>
...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li class="click"><a href="addcourse.php"><span><img src="images/t01.png" /></span>添加</a></li>
    ...
    <td class="imgtd"><img src="<?php echo $value['cover']; ?>" width="80" height="60" /></td>
    <td><a href="#"><?php echo $value['name']; ?></a></td>
    <td><?php echo $value['ontime']; ?></td>
    <td><?php echo date('Y-m-d',$value['addtime']); ?></td>
    <td><?php echo $value['classroom']; ?></td>
    <td><?php echo $value['status']==1?'上架':($value['status']==2?'下架':'删除'); ?></td>
    <td>
          <a href="upcourse.php?id=<?php echo $value['id']; ?>" class="tablelink">编辑</a>
       ...
       for(var i=0;i<del.length;i++){
            del[i].onclick = function(){
                ...
                if(this.getAttribute('del-data')!=null){
                   delData[0] = this.getAttribute('del-data');
                }else{
                    for(var i=0;i<list.length;i++){
                        if(list[i].checked){
                            delData.push(list[i].value);
                        }
                    }
                }
                $(".sure").click(function(){
                  $(".tip").fadeOut(100);
                  location.href="show-course.php?del="+delData.join(',');
                });
            }
       }
	</script>
